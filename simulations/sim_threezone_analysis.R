# =====================================================================
# THREE-ZONE DESIGN: analysis, metrics, maps, residuals
# ---------------------------------------------------------------------
# Run after sim_threezone_generate.R, which leaves in the environment:
#   sim_data, z_s, zone, in_high, grid, lp1, lp2, split1_thresh,
#   split2_thresh, lp1_of_s, lp2_of_s, z_levels, right_side,
#   b_wbc, b_tpi
#
# Settings match the LeukSurv application and the two-zone rerun:
...
# ratio. AUC is omitted: it needs a single ROC curve, which three
...
stopifnot(exists("sim_data"), exists("zone"), exists("grid"),
          exists("split1_thresh"), exists("split2_thresh"), exists("z_s"))

alldata <- sim_data
n       <- nrow(alldata)
truth_k <- as.integer(factor(alldata$zone, levels = c("low", "medium", "high")))
...
# Far-boundary set. Two boundaries: split 1 everywhere, split 2 on the
# right side only. Each margin is scaled by the SD of its own linear
# predictor so the two are comparable, then a patient's margin is the
# distance to whichever boundary is nearer.
m1 <- abs(lp1 - split1_thresh) / sd(lp1)
m2 <- abs(lp2 - split2_thresh) / sd(lp2)
m2[!right_side] <- Inf              # split 2 does not bound the left side
margin <- pmin(m1, m2)
...
Dip.cl <- DipolarSurvivalTree_MixedGaussPolyLinearKernel_recpom$new(
  ...
  Kconstant = 0, Kpoly_order = 1,metric="kernel",
...
# 5. Metrics
# =====================================================================
cat("\n===== Zone recovery (3-means, clusters ordered by mean risk) =====\n")
s.sp <- score_multiclass(risk.sp, lrr(pt.sp), "spatial-only",
                         truth_k, far, zc, span_true, k = 3)
...
ggsave("fig_threezone_2x2.pdf", fig, width = 11, height = 9)
ggsave("fig_threezone_2x2.png", fig, width = 11, height = 9, dpi = 300)
cat("\nFigure written: fig_threezone_2x2.pdf\n")

# =====================================================================
# 7. Residual diagnostics
# =====================================================================
d <- alldata
d$zone <- factor(d$zone, levels = c("low", "medium", "high"))
...
for (zz in levels(d$zone)) {
  ...
  cat(sprintf("%-8s %8.3f %8.3f %12.3f %12.3f\n",
              zz, mean(sub$e_resid), median(sub$e_resid),
              cor(sub$e_resid, sub$num_ycoord), r2))
}
cat("\nPairwise Mann-Whitney (higher frailty should give smaller residuals):\n")
for (pr in list(c("high","medium"), c("medium","low"), c("high","low"))) {
  sub <- d[d$zone %in% pr, ]
  sub$zone <- factor(sub$zone, levels = pr)
  pv <- wilcox.test(e_resid ~ zone, data = sub, alternative = "less")$p.value
  cat(sprintf("  %-7s < %-7s : p = %.3g\n", pr[1], pr[2], pv))
}
...
p_rtpi <- ggplot(d, aes(num_tpi, e_resid, color = zone)) +
  ...
  theme_minimal()
ggsave("fig_resid_vs_tpi_threezone.pdf", p_rtpi, width = 5.5, height = 4)
cat("Figure written: fig_resid_vs_tpi_threezone.pdf\n")

# =====================================================================
# 9. Save
# =====================================================================
objs <- c("sim_data", "alldata", "z_s", "zone", "grid", "lp1", "lp2",
          "split1_thresh", "split2_thresh", "right_side", "b_wbc", "b_tpi",
          "risk.sp", "risk.ts", "pt.sp", "pt.ts", "s.sp", "s.ts",
          "span_true", "margin", "far", "p_rtpi")
if (exists("risk.sb") && !is.null(risk.sb)) objs <- c(objs, "risk.sb")
if (exists("s.sb")) objs <- c(objs, "s.sb")
save(list = objs, file = "sim_threezone_key_objects.RData")

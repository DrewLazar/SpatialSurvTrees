...
DipolarSurvivalTree_MixedGaussPolyLinearKernel_recpom = R6Class(
  
  
  
  classname = "DipolarSurvivalTree_MixedGaussPolyLinearKernel_recpom",
  ...
  public = list(
    ...
    epsilon = NA_real_, # uniform margin for piecewise linear function (active value; may change per node if adaptive)
    epsilon_root = NA_real_, # root-node margin (cached at initialisation; used as the floor reference)
    probs = NA_real_, # quantile probability for data-driven epsilon (e.g. 0.20)
    adaptive = FALSE, # if TRUE, recompute epsilon locally at every non-terminal node
    epsilon_floor_alpha = NA_real_, # floor factor: epsilon_node >= epsilon_floor_alpha * epsilon_root
    metric = "euclidean", # distance metric for epsilon calibration: "euclidean" or "kernel"
    kappa = NA_real_,
    ncovariatestosearch = NA_integer_,
    gausscovariates_index = NA_integer_,
    gaussweight = NA_real_,
    polycovariates_index = NA_integer_,
    polyweight = NA_real_,
    linearcovariates_index = NA_integer_,
    linearweight = NA_real_,
    ...
    quantiles = c(.35,.65), tolerance = 10^-3,
    epsilon = NULL,            # manual scalar override; if NULL, root epsilon is computed from data via `probs`
    probs = 0.20,              # quantile prob for data-driven epsilon (root, and per-node if adaptive)
    adaptive = FALSE,          # if TRUE, recompute epsilon at every non-terminal node
    epsilon_floor_alpha = 0.2, # floor: epsilon_node >= epsilon_floor_alpha * epsilon_root  (set 0 to disable)
    metric = c("euclidean", "kernel"), # distance used to calibrate epsilon
    kappa = 1, nsize = 10L,
    pureweight = 1, mixedweight = 1,
    Ksigma = 1, Kconstant = 1, Kpoly_order = 2,
    ncovariatestosearch = 10, gaussweight = 1, gausscovariates_index, polyweight = 1, polycovariates_index, linearweight = 1, linearcovariates_index
    ) {
      ...
      # Resolve the distance metric early; it is referenced in the root-epsilon block below.
      metric <- match.arg(metric)
      self$metric = metric
      
      # --- Kernel setup -------------------------------------------------------
      # Must run BEFORE the adaptive-margin block, because computing root
      # epsilon under metric = "kernel" requires self$K.bicovariates.
      self$kappa = kappa
      self$nsize = nsize
      
      self$gausscovariates_index = gausscovariates_index
      self$gaussweight = gaussweight
      
      self$polycovariates_index = polycovariates_index
      self$polyweight = polyweight
      
      self$linearcovariates_index = linearcovariates_index
      self$linearweight = linearweight
      
      if(linearweight < 0 || linearweight > 1) {
        stop("linear kernel weight must be 0-1")
      }
      ...
      # Full-data Gram matrix; needed both for tree growing and (if metric =
      # "kernel") for root-epsilon calibration just below.
      ...
      # --- Adaptive margin setup ----------------------------------------------
      # Store new options
      self$probs               = probs
      self$adaptive            = adaptive
      self$epsilon_floor_alpha = epsilon_floor_alpha
      
      # Validate
      if (!is.numeric(probs) || length(probs) != 1L || probs <= 0 || probs >= 1) {
        stop("\"probs\" must be a single number strictly between 0 and 1")
      }
      if (!is.logical(adaptive) || length(adaptive) != 1L || is.na(adaptive)) {
        stop("\"adaptive\" must be TRUE or FALSE")
      }
      if (!is.numeric(epsilon_floor_alpha) || length(epsilon_floor_alpha) != 1L ||
          epsilon_floor_alpha < 0) {
        stop("\"epsilon_floor_alpha\" must be a non-negative number")
      }
      
      # Compute root epsilon unless the user supplied a manual scalar override.
      if (is.null(epsilon)) {
        if (length(covariates) == 0L) {
          stop("Cannot compute data-driven epsilon when \"covariates\" is empty")
        }
        if (self$metric == "euclidean") {
          # Input-space pairwise Euclidean distances on the raw covariates.
          Xroot    <- as.matrix(traindata[, covariates])
          distroot <- c(dist(Xroot))
        } else {
          # Kernel-induced distances derived from the full-data Gram matrix:
          # d(x_i, x_j) = sqrt(K[i,i] + K[j,j] - 2 K[i,j]).
          distroot <- self$gram_to_dist_vec(self$K.bicovariates)
        }
        distroot <- distroot[distroot != 0]
        if (length(distroot) == 0L) {
          stop("All pairwise distances are zero; cannot compute epsilon from data")
        }
        self$epsilon_root <- as.numeric(quantile(distroot, probs = probs))
      } else {
        if (!is.numeric(epsilon) || length(epsilon) != 1L || epsilon <= 0) {
          stop("\"epsilon\" override must be a single positive number, or NULL")
        }
        self$epsilon_root <- as.numeric(epsilon)
      }
      # Active epsilon used by the QP at the current node. In non-adaptive mode
      # this remains constant; in adaptive mode it is overwritten in
      # optimal_w0_mupmdiff() before each split.
      self$epsilon <- self$epsilon_root
      # ------------------------------------------------------------------------
    },
    
    
    K.bi = function(X) {
      # gindex = (1:length(self$covariates))[-c(self$polycovariates_index,
      #                                         self$linearcovariates_index)]
      Xl = as.matrix(X[, self$linearcovariates_index])
      Xp = as.matrix(X[, self$polycovariates_index])
      Xg = as.matrix(X[, self$gausscovariates_index])
      # XminXsqr_l = apply(Xl, MARGIN = 1, FUN = function(xi) {
      #   apply(Xl, MARGIN = 1, FUN = function(xj) {
      #     sum(xi * xj)
      #   })
      # })
      XminXsqr_g = apply(Xg, MARGIN = 1, FUN = function(xi) {
        apply(Xg, MARGIN = 1, FUN = function(xj) {
          sum((xi - xj)^2)
        })
      })
      # print(XminXsqr_l)
      # print(eigen(XminXsqr_l))
      # print(XminXsqr_g)
      # print(exp(-XminXsqr_g/(2*self$Ksigma)))
      # print(eigen(exp(-XminXsqr_g/(2*self$Ksigma))))
      # U = (self$linearweight)*XminXsqr_l + (1-self$linearweight)*exp(-XminXsqr_g/(2*self$Ksigma))
      # print(U)
      # print(eigen(U))
      # U
      # print(Xl%*%t(Xl))
      # print((Xp%*%t(Xp) + self$Kconstant))
      (self$linearweight)*(Xl%*%t(Xl)) + (self$polyweight)*(Xp%*%t(Xp) + self$Kconstant)^self$Kpoly_order + (self$gaussweight)*exp(-XminXsqr_g/(2*self$Ksigma))
    },
    ...
    K = function(X, x) {
      ...
      Xp = as.matrix(X[, self$polycovariates_index])
      Xg = as.matrix(X[, self$gausscovariates_index])
      xl = x[self$linearcovariates_index]
      xp = x[self$polycovariates_index]
      xg = x[self$gausscovariates_index]
      # Xminxsqr_l = apply(Xl, MARGIN = 1, FUN = function(xi) {
      #   sum(xi * xl)
      # })
      Xminxsqr_g = apply(Xg, MARGIN = 1, FUN = function(xi) {
      ...
      (self$linearweight)*(Xl%*%xl) + (self$polyweight)*(Xp%*%xp + self$Kconstant)^self$Kpoly_order + (self$gaussweight)*exp(-Xminxsqr_g/(2*self$Ksigma))
    },
    ...
    # CREATE "PURE OR MIXED" MATRIX
    createsubpoms = function() {
      lowerQ = self$quantiles[1]
      upperQ = self$quantiles[2]
      # Make pairwise distance vector while accounting for censoring
      subsetX = strtoi(rownames(private$X))
      ...
      for (i in 1:(N-1)) {
        # survival information of i-th data point
        xitime = datasubset[i, self$time]
        xicensor = datasubset[i, self$censor]
        for (j in (i+1):N) {
          # survival information of j-th data point
          xjtime = datasubset[j, self$time]
          xjcensor = datasubset[j, self$censor]
          # account for censoring as follows:
          if (
            (xicensor == 1) && (xjcensor == 1) ||
            ((xicensor == 0) && (xjcensor == 1) && (xitime > xjtime)) ||
    ...
    createtreenode = function(node) {
      ...
      if (n > 2*(self$nsize)) {
        ...
        if (nl > self$nsize && nr > self$nsize) {
          splits <- lrnodes[[3]]
    ...
    optimal_w0_mupmdiff = function() {
      private$subpoms = self$createsubpoms()
      # If adaptive, recompute the hinge-loss margin from this node's covariates
      # before optimisation. Mirrors createsubpoms in spirit: a per-node
      # adaptation that uses only the data inside the node.
      if (isTRUE(self$adaptive)) {
        self$epsilon <- self$compute_local_epsilon()
      }
      w0_mupmdiff = private$optimizer()
      return(w0_mupmdiff)
    },
    
    # COMPUTE LOCAL EPSILON FROM CURRENT NODE'S COVARIATES OR GRAM MATRIX
    # epsilon_node = max( quantile_{probs}(local pairwise distances),
    #                     epsilon_floor_alpha * epsilon_root )
    # The metric controls which pairwise distance is used:
    #   - "euclidean": dist() on the raw covariate matrix from private$X
    #   - "kernel":    sqrt(K[i,i] + K[j,j] - 2 K[i,j]) from private$K.XX
    # Falls back to the floor alone when the node has < 2 rows or all
    # distances are zero.
    compute_local_epsilon = function() {
      floor_eps <- self$epsilon_floor_alpha * self$epsilon_root
      if (self$metric == "euclidean") {
        Xnode <- as.matrix(private$X[, self$covariates])
        if (nrow(Xnode) < 2L) return(floor_eps)
        d <- c(dist(Xnode))
      } else {
        # metric == "kernel"
        K <- private$K.XX
        if (nrow(K) < 2L) return(floor_eps)
        d <- self$gram_to_dist_vec(K)
      }
      d <- d[d != 0]
      if (length(d) == 0L) return(floor_eps)
      local_eps <- as.numeric(quantile(d, probs = self$probs))
      return(max(local_eps, floor_eps))
    },
    
    # CONVERT A GRAM MATRIX INTO A VECTOR OF PAIRWISE KERNEL-INDUCED DISTANCES
    # d(x_i, x_j) = sqrt( K[i,i] + K[j,j] - 2 K[i,j] )
    # Returns the upper-triangle (excluding diagonal) as a numeric vector,
    # mirroring the length and role of c(dist(X)) in the Euclidean case.
    # Negative values from numerical subtraction error are clipped to zero
    # before the square root.
    gram_to_dist_vec = function(K) {
      diagK <- diag(K)
      D2    <- outer(diagK, diagK, "+") - 2 * K
      D2    <- D2[upper.tri(D2)]
      D2    <- pmax(D2, 0)
      sqrt(D2)
    },

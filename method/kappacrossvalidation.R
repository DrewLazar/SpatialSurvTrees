...
bootfun = function(data, R, k){
  n = nrow(data)
  # replicate returns a matrix by default, simplify=FALSE keeps it as a List
  samples <- replicate(k, sample(1:n, R, replace = TRUE), simplify = FALSE)
  # Optional: Assign names to the list elements to match the feel of 'split'
  names(samples) <- 1:k
  return(samples)
}
